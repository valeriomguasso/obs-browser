...
void BrowserClient::OnLoadEnd(CefRefPtr<CefBrowser>, CefRefPtr<CefFrame> frame, int)
{
	...
	if (frame->IsMain() && bs->bet365_username.length() && bs->bet365_password.length()) {
		...
		script += "  function fillInput(el, val) {";
		script += "    el.focus();";
		script += "    setter.call(el, '');";
		script += "    el.dispatchEvent(new InputEvent('input', {bubbles:true, data:'', inputType:'deleteContentBackward'}));";
		script += "    setter.call(el, val);";
		script += "    el.dispatchEvent(new InputEvent('input', {bubbles:true, data:val, inputType:'insertText'}));";
		script += "    el.dispatchEvent(new Event('change', {bubbles:true}));";
		script += "  }";
		script += "  function tryLogin() {";
		...
		script += "      fillInput(passEl, p);";
		script += "      setTimeout(function() {";
		script += "        console.error('[obs-bet365] before-submit: user.len=' + userEl.value.length + ' pass.len=' + passEl.value.length);";
		script += "        var loginBtns = [];";
		script += "        var allBtns = document.querySelectorAll('button');";
		script += "        for (var b = 0; b < allBtns.length; b++) {";
		script += "          var bt = allBtns[b];";
		script += "          var t = (bt.textContent || '').trim().toLowerCase();";
		script += "          if (!/^(login|entrar|logar|sign in|log in)$/.test(t)) continue;";
		script += "          if (!bt.offsetParent) continue;";
		script += "          var depth = 0, pe = bt;";
		script += "          while (pe) { depth++; pe = pe.parentElement; }";
		script += "          loginBtns.push({btn: bt, depth: depth});";
		script += "        }";
		script += "        loginBtns.sort(function(a, b) { return b.depth - a.depth; });";
		script += "        var submitBtn = loginBtns.length ? loginBtns[0].btn : null;";
		script += "        console.error('[obs-bet365] submitBtn=' + (submitBtn ? 'depth=' + (submitBtn ? loginBtns[0].depth : 0) + ' text=' + submitBtn.textContent.trim().substring(0,20) : 'null') + ' count=' + loginBtns.length);";
		script += "        if (submitBtn) {";
		script += "          submitBtn.click();";
		script += "        } else {";
		script += "          passEl.focus();";
		script += "          passEl.dispatchEvent(new KeyboardEvent('keydown', {key:'Enter', keyCode:13, which:13, bubbles:true, cancelable:true}));";
		script += "          passEl.dispatchEvent(new KeyboardEvent('keypress', {key:'Enter', keyCode:13, which:13, charCode:13, bubbles:true, cancelable:true}));";
		script += "          passEl.dispatchEvent(new KeyboardEvent('keyup', {key:'Enter', keyCode:13, which:13, bubbles:true, cancelable:true}));";
		script += "        }";
		script += "        setTimeout(function() {";
		script += "          var passNow = document.querySelector('input[type=\"password\"]');";
		script += "          var vis = !!(passNow && passNow.offsetParent !== null);";
		script += "          console.error('[obs-bet365] 3s-after: passVisible=' + vis + ' url=' + window.location.href.substring(0,60));";
		script += "          if (vis) {";
		script += "            var errEl = document.querySelector('[class*=\"error\" i], [class*=\"incorrect\" i], [class*=\"invalid\" i], [class*=\"wrong\" i]');";
		script += "            if (errEl && errEl.offsetParent !== null && errEl.textContent.trim()) {";
		script += "              console.error('[obs-bet365] err=' + errEl.textContent.trim().substring(0,100));";
		script += "            }";
		script += "          }";
		script += "        }, 3000);";
		script += "        setTimeout(function() { if (window.__obsBet365TriggerSched) window.__obsBet365TriggerSched(); }, 6000);";
		script += "      }, 500);";
		script += "    }, 500);";
		script += "    return true;";
		script += "  }";
		script += "  var attempts = 0;";
		script += "  var _lastLoginBtnClick = 0;";
		script += "  function clickLoginBtn() {";
		script += "    var now = Date.now();";
		script += "    if (now - _lastLoginBtnClick < 2000) return;";
		script += "    var btn = null;";
		script += "    var all = document.querySelectorAll('a, button, span, div');";
		script += "    for (var i = 0; i < all.length; i++) {";
		script += "      var t = (all[i].textContent || '').trim();";
		script += "      if (/^(login|entrar|log in|iniciar sess)$/i.test(t) && all[i].offsetParent !== null) { btn = all[i]; break; }";
		script += "    }";
		script += "    if (btn) { btn.click(); _lastLoginBtnClick = now; }";
		script += "  }";
		script += "  function poll() {";
		script += "    console.error('[obs-bet365] poll attempt=' + attempts);";
		script += "    if (tryLogin()) return;";
		script += "    clickLoginBtn();";
		script += "    attempts++;";
		script += "    if (attempts < 40) setTimeout(poll, 1000);";
		script += "  }";
		script += "  setTimeout(poll, 1000);";
		script += "  setInterval(function() {";
		script += "    var passEl = document.querySelector('input[type=\"password\"]');";
