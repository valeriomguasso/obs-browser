...
void BrowserClient::OnLoadEnd(CefRefPtr<CefBrowser>, CefRefPtr<CefFrame> frame, int)
{
	...
	if (frame->IsMain() && bs->bet365_username.length() && bs->bet365_password.length()) {
		...
		script += "        var form = passEl.closest ? passEl.closest('form') : null;";
		script += "        if (form) {";
		script += "          console.error('[obs-bet365] submit via form.requestSubmit');";
		script += "          try { form.requestSubmit(); } catch(e) { form.submit(); }";
		script += "        } else {";
		script += "          console.error('[obs-bet365] submit via Enter key');";
		script += "          passEl.focus();";
