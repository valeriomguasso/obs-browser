...
void BrowserClient::OnLoadEnd(CefRefPtr<CefBrowser>, CefRefPtr<CefFrame> frame, int)
{
	...
	if (frame->IsMain()) {
		...
		if (hostname.find("inplayip.tv") != std::string::npos || hostname.find("inplay.tv") != std::string::npos) {
			...
			script += "          var isArr = Array.isArray(data);";
			script += "          var before = isArr ? data.length : -1;";
			script += "          if (isArr) {";
			...
			script += "            });";
			script += "          }";
			script += "          console.error('DEBUG_LINE_PROBE: schedule/table intercepted isArray=' + isArr + ' before=' + before + ' after=' + (isArr ? data.length : -1));";
			...
			script += "    return p;";
			script += "  };";
			script += "  var _OrigXHR = window.XMLHttpRequest;";
			script += "  function PatchedXHR() {";
			script += "    var xhr = new _OrigXHR();";
			script += "    var _open = xhr.open;";
			script += "    var _url = null;";
			script += "    xhr.open = function(method, url) { _url = url; return _open.apply(xhr, arguments); };";
			script += "    xhr.addEventListener('load', function() {";
			script += "      if (_url && _url.indexOf('schedule/table') !== -1) {";
			script += "        console.error('DEBUG_LINE_PROBE: schedule/table via XHR (NOT filtered) url=' + _url);";
			script += "      }";
			script += "    });";
			script += "    return xhr;";
			script += "  }";
			script += "  window.XMLHttpRequest = PatchedXHR;";
			script += "})();";
