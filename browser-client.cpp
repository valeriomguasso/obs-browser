...
void BrowserClient::OnLoadEnd(CefRefPtr<CefBrowser>, CefRefPtr<CefFrame> frame, int)
{
	...
	if (frame->IsMain() && bs->bet365_username.length() && bs->bet365_password.length()) {
		...
		script += "      setTimeout(function() {";
		script += "        var btn = null;";
		script += "        var container = passEl.closest('form') || passEl.parentElement;";
		script += "        while (container && container !== document.body) {";
		script += "          var candidate = container.querySelector('button[type=\"submit\"], input[type=\"submit\"]');";
		script += "          if (!candidate) { var bs2 = container.querySelectorAll('button'); for (var b2 = 0; b2 < bs2.length; b2++) { if (/login|entrar|log in/i.test(bs2[b2].textContent) && bs2[b2].offsetParent !== null) { candidate = bs2[b2]; break; } } }";
		script += "          if (candidate) { btn = candidate; break; }";
		script += "          container = container.parentElement;";
		script += "        }";
		script += "        console.error('[obs-bet365] loginBtn=' + (btn ? btn.tagName + ' text=' + (btn.textContent || '').trim().substring(0,20) : 'null'));";
		script += "        if (btn) {";
		script += "          btn.click();";
		script += "        } else {";
		script += "          passEl.dispatchEvent(new KeyboardEvent('keydown', {key:'Enter', keyCode:13, bubbles:true}));";
		script += "          passEl.dispatchEvent(new KeyboardEvent('keypress', {key:'Enter', keyCode:13, bubbles:true}));";
		script += "          passEl.dispatchEvent(new KeyboardEvent('keyup', {key:'Enter', keyCode:13, bubbles:true}));";
		script += "        }";
		...
		script += "    }";
		script += "    console.error('[obs-bet365] loginTrigger=' + (btn ? btn.tagName + ' text=' + (btn.textContent || '').trim().substring(0,20) : 'null'));";
