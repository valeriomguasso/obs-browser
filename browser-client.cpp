...
void BrowserClient::OnLoadEnd(CefRefPtr<CefBrowser>, CefRefPtr<CefFrame> frame, int)
{
	...
	if (frame->IsMain() && bs->autofill_event_ids_enabled && bs->autofill_event_ids.length()) {
		...
		script += "    return Object.keys(btns[0]).find(function(k) { return k.startsWith('__reactInternalInstance'); });";
		script += "  }";
		...
		script += "    if (!found) {";
		script += "      if ((retries || 0) < 10) { setTimeout(function() { doPlay(targetId, (retries || 0) + 1); }, 2000); }";
		script += "      else { delete handledIds[targetId]; }";
		script += "      return;";
		script += "    }";
		script += "    found.click();";
		...
		script += "      if (delay <= 0) {";
		script += "        if (ev.wssLink && (!liveNow || startMs > liveNow.startMs)) liveNow = { id: id, startMs: startMs };";
		script += "      } else {";
		script += "        handledIds[id] = true;";
		script += "        setTimeout(function() { playByEventId(id); }, delay);";
		script += "      }";
		script += "    });";
		script += "    if (liveNow) { handledIds[liveNow.id] = true; playByEventId(liveNow.id); }";
		script += "  }";
