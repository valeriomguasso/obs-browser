...
void BrowserClient::OnLoadEnd(CefRefPtr<CefBrowser>, CefRefPtr<CefFrame> frame, int)
{
	...
	if (frame->IsMain()) {
		...
		if (hostname.find("inplayip.tv") != std::string::npos || hostname.find("inplay.tv") != std::string::npos) {
			...
			script += "  function isDateScopedBody(bodyStr) {";
			script += "    try {";
			script += "      var b = JSON.parse(bodyStr);";
			script += "      return !!(b && b.filters && b.filters.searchDate);";
			script += "    } catch (e) { return false; }";
			script += "  }";
			script += "  function filterSchedule(data) {";
			script += "    if (!Array.isArray(data)) return data;";
			script += "    var cutoff = Date.now() - 24 * 60 * 60 * 1000;";
			script += "    return data.filter(function(g) {";
			...
			script += "    if (url && url.indexOf('schedule/table') !== -1 && opts && typeof opts.body === 'string' && isDateScopedBody(opts.body)) {";
			...
			script += "    return p;";
			script += "  };";
			script += "  var _xhrOpen = XMLHttpRequest.prototype.open;";
			script += "  XMLHttpRequest.prototype.open = function(method, url) {";
			script += "    this.__obsScheduleReq = typeof url === 'string' && url.indexOf('schedule/table') !== -1;";
			script += "    this.__obsScheduleScoped = false;";
			script += "    return _xhrOpen.apply(this, arguments);";
			script += "  };";
			script += "  var _xhrSend = XMLHttpRequest.prototype.send;";
			script += "  XMLHttpRequest.prototype.send = function(body) {";
			script += "    if (this.__obsScheduleReq && typeof body === 'string') {";
			script += "      this.__obsScheduleScoped = isDateScopedBody(body);";
			script += "    }";
			script += "    return _xhrSend.apply(this, arguments);";
			script += "  };";
			...
			script += "  Object.defineProperty(XMLHttpRequest.prototype, 'responseText', { configurable: true, get: function() {";
			script += "    var text = rtDesc.get.call(this);";
			script += "    return (this.__obsScheduleReq && this.__obsScheduleScoped && text) ? filterXhrText(text) : text;";
			script += "  }});";
			script += "  var rDesc = Object.getOwnPropertyDescriptor(XMLHttpRequest.prototype, 'response');";
			script += "  Object.defineProperty(XMLHttpRequest.prototype, 'response', { configurable: true, get: function() {";
			script += "    var res = rDesc.get.call(this);";
			script += "    if (!this.__obsScheduleReq || !this.__obsScheduleScoped) return res;";
			script += "    if (typeof res === 'string') return filterXhrText(res);";
	...
	if (frame->IsMain() && bs->superbet_event_ids.length()) {
		...
		script += "  }";
		script += "  function maximizeByPath() {";
		script += "    var target = document.querySelector('.video-player');";
		script += "    if (!target) { var v = document.querySelector('video'); if (!v) return; target = v.parentElement; }";
		script += "    console.error('[obs-superbet] maximizando via path: ' + target.className.substring(0,40));";
		script += "    var el = target;";
		script += "    while (el && el !== document.body) {";
		script += "      var parent = el.parentElement;";
		script += "      if (parent) {";
		script += "        for (var i = 0; i < parent.children.length; i++) {";
		script += "          if (parent.children[i] !== el) parent.children[i].style.cssText += 'display:none!important;';";
		script += "        }";
		script += "        parent.style.cssText += 'width:100%!important;height:100%!important;max-width:none!important;max-height:none!important;overflow:visible!important;position:static!important;';";
		script += "      }";
		script += "      el = parent;";
		script += "    }";
		script += "    target.style.cssText += 'width:100%!important;height:100%!important;max-width:none!important;max-height:none!important;';";
		script += "    var video = target.querySelector('video') || document.querySelector('video');";
		script += "    if (video) video.style.cssText += 'width:100%!important;height:100%!important;object-fit:contain!important;display:block!important;';";
		script += "    document.documentElement.style.cssText = 'width:100%;height:100%;overflow:hidden;background:#000;margin:0;padding:0;';";
		script += "    document.body.style.cssText = 'width:100%;height:100%;overflow:hidden;background:#000;margin:0;padding:0;';";
		script += "  }";
		script += "  function waitAndMaximize() {";
		script += "    var tries = 0;";
		script += "    console.error('[obs-superbet] aguardando player...');";
		script += "    var iv = setInterval(function() {";
		script += "      tries++;";
		script += "      var video = document.querySelector('video');";
		script += "      if (video && video.videoWidth > 0 && video.readyState >= 2) {";
		script += "        clearInterval(iv);";
		script += "        console.error('[obs-superbet] video pronto (' + video.videoWidth + 'x' + video.videoHeight + ')');";
		script += "        maximizeByPath();";
		script += "        setTimeout(maximizeByPath, 1000);";
		script += "        return;";
		script += "      }";
		script += "      if (tries >= 60) { clearInterval(iv); console.error('[obs-superbet] timeout aguardando video'); }";
		script += "    }, 500);";
		script += "  }";
		...
		script += "    var iv = setInterval(function() {";
		script += "      tries++;";
		script += "      var labels = document.querySelectorAll('.e2e-sds-button__label, .sds-button-text__label');";
		script += "      for (var i = 0; i < labels.length; i++) {";
		script += "        var t = labels[i].textContent.trim();";
		script += "        if (t === 'Assista' || t === 'Assistir' || t === 'Watch') {";
		script += "          var ab = labels[i].closest ? labels[i].closest('button') : labels[i].parentElement;";
		script += "          if (ab) { clearInterval(iv); console.error('[obs-superbet] clicando Assista'); ab.click(); setTimeout(waitAndMaximize, 1500); return; }";
		script += "        }";
		script += "      }";
		script += "      if (tries >= 20) { clearInterval(iv); console.error('[obs-superbet] Assista nao encontrado apos 10s'); }";
		script += "    }, 500);";
		script += "  }";
		...
		script += "    var iv = setInterval(function() {";
		script += "      tries++;";
		script += "      var labels = document.querySelectorAll('.sds-button-text__label, .e2e-sds-button__label');";
		script += "      for (var i = 0; i < labels.length; i++) {";
		script += "        var t = labels[i].textContent.trim();";
		script += "        if (t === 'Transmissão' || t === 'Ao Vivo' || t === 'Live') {";
		script += "          var btn = labels[i].closest ? labels[i].closest('button') : labels[i].parentElement;";
		script += "          if (btn) { clearInterval(iv); console.error('[obs-superbet] clicando Transmissao'); btn.click(); setTimeout(clickAssista, 1000); return; }";
		script += "        }";
		script += "      }";
		script += "      if (tries >= 30) { clearInterval(iv); console.error('[obs-superbet] Transmissao nao encontrado apos 15s, tentando Assista direto'); clickAssista(); }";
		script += "    }, 500);";
		script += "  }";
	...
	if (frame->IsMain()) {
		...
		if (frameUrl.find("performgroup.com") != std::string::npos ||
			...
			script += "  console.error('[obs-superbet] fullscreen script iniciado');";
			script += "  var maximized = false;";
			script += "  var maxSelectors = [";
			script += "    '.fullscreen-icon',";
			script += "    'button[aria-label*=\"full\" i]',";
			script += "    '[class*=\"fullscreen\"]',";
			script += "    'button[title*=\"full\" i]',";
			script += "    '.vjs-fullscreen-control',";
			script += "    '[data-id=\"fullscreen\"]'";
			script += "  ];";
			script += "  function tryMaxBtn() {";
			script += "    for (var s = 0; s < maxSelectors.length; s++) {";
			script += "      try {";
			script += "        var els = document.querySelectorAll(maxSelectors[s]);";
			script += "        for (var i = 0; i < els.length; i++) {";
			script += "          var el = els[i];";
			script += "          if (!el.offsetParent) continue;";
			script += "          console.error('[obs-superbet] max btn: sel=' + maxSelectors[s] + ' tag=' + el.tagName + ' cls=' + el.className.substring(0,50));";
			script += "          el.click();";
			script += "          maximized = true;";
			script += "          return true;";
			script += "        }";
			script += "      } catch(e) {}";
			script += "    }";
			script += "    return false;";
			script += "  }";
			script += "  function applyCss() {";
			script += "    var ds = document.documentElement.style;";
			script += "    ds.margin = '0'; ds.padding = '0'; ds.overflow = 'hidden';";
			script += "    ds.width = '100%'; ds.height = '100%'; ds.background = '#000';";
			...
			script += "    console.error('[obs-superbet] max btn nao encontrado, aplicando CSS');";
			script += "    applyCss();";
			script += "  }";
			script += "  fill();";
			script += "  setTimeout(fill, 1000);";
			script += "  setTimeout(fill, 3000);";
			script += "  setTimeout(fill, 6000);";
			script += "  setTimeout(fill, 10000);";
			script += "  var mo = new MutationObserver(function() { if (!maximized) fill(); });";
			script += "  setTimeout(function() { if (document.body) mo.observe(document.body, {childList:true, subtree:true}); }, 500);";
			script += "})();";
