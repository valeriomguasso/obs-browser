...
void BrowserClient::OnLoadEnd(CefRefPtr<CefBrowser>, CefRefPtr<CefFrame> frame, int)
{
	...
	if (frame->IsMain() && bs->bet365_username.length() && bs->bet365_password.length()) {
		...
		script += "        var submitBtn = null;";
		script += "        var el = passEl.parentElement;";
		script += "        for (var d = 0; d < 8 && el && !submitBtn; d++) {";
		script += "          var btns = el.querySelectorAll('button');";
		script += "          for (var b = 0; b < btns.length; b++) {";
		script += "            var t = (btns[b].textContent || '').trim().toLowerCase();";
		script += "            if (t === '' || /show|hide|mostr|ocult|toggle/i.test(t)) continue;";
		script += "            if (btns[b].offsetParent !== null) { submitBtn = btns[b]; break; }";
		script += "          }";
		script += "          if (!submitBtn) el = el.parentElement;";
		script += "        }";
		script += "        console.error('[obs-bet365] submitBtn=' + (submitBtn ? submitBtn.textContent.trim().substring(0,20) : 'null'));";
		script += "        if (submitBtn) {";
